...
## Carga de datos (solo se los archivos existen; ajusta la condición según tu necesidad)
...
# Función que retorna el nombre de la tienda si se cumple la condición de distancia
def obtener_tienda_con_estado(lat, lon, branches):
    for branch in branches:
        branch_umbral = branch.get("umbral", 0.2)  # Umbral específico para cada tienda, por defecto 0.2 km
        if haversine(lat, lon, branch['lat'], branch['lon']) <= branch_umbral:
            return branch['name']
    return ""

# Función para detectar vehículos con posible necesidad de mantenimiento
...
if nox_exists:
    ...
    ax = sns.boxplot(data=nox_procesado, x="vehicle_number", y="NOx", hue="clasificacion_velocidad", dodge=True)
    
    sns.stripplot(data=nox_procesado, x="vehicle_number", y="NOx", hue="clasificacion_velocidad", 
                  dodge=True, jitter=True, alpha=0.5, marker="o", size=4, color=".3")
    
    plt.title("Distribuição de NOx por Veículo e Classificação de Velocidade", fontsize=14)
    plt.xlabel("Número do Veículo", fontsize=12)
    plt.ylabel("Nível de NOx", fontsize=12)
    plt.xticks(rotation=90)
    
    handles, labels = ax.get_legend_handles_labels()
    plt.legend(handles[:3], labels[:3], title="Classificação de Velocidade", bbox_to_anchor=(1.05, 1), loc='upper left')
    
    plt.savefig("boxplot_nox_por_veiculo_e_velocidade.png", bbox_inches="tight")
    plt.show()
    
    print("✅ Boxplot guardado como 'boxplot_nox_por_veiculo_e_velocidade.png'")
